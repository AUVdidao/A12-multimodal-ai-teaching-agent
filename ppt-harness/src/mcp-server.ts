...
import { HarnessError, PresentationJobRequest } from "./domain.js";
...
async function main(): Promise<void> {
  ...
  server.tool("generate_teaching_presentation", "Create an asynchronous teaching presentation task", {
    requestId: z.string().min(1).max(128), projectId: z.number().int().positive(), jobSnapshot: z.record(z.unknown()),
  ...
  }, async input => text({ task: await harness.workflow.submit(input as unknown as PresentationJobRequest) }));
