...
public record ModelExecutionContext(
    ...
    public ModelExecutionContext {
        ...
        if (modelConnectionId != null && modelConnectionId <= 0) {
            throw new IllegalArgumentException("modelConnectionId must be positive");
        }
        required(purpose, "purpose", MAX_ID_CHARS);
        metadata = boundedMetadata(metadata);
    }

    public ModelExecutionContext(long projectId, String actorId, String actorRole, String traceId, String runId,
                                 String agentName, long remainingBudgetMs, Map<String, String> metadata) {
        this(projectId, actorId, actorRole, traceId, runId, agentName, remainingBudgetMs, metadata, null, "UNSPECIFIED");
    }
