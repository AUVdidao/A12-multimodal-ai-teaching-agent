...
import com.auvdidao.a12teachingagent.clarification.MissingField;
...
@Service
public class MockAIWorkflowGateway implements AIWorkflowGateway {
    ...
    @Override
    public ClarificationResult checkRequirementClarification(Long projectId, ClarificationCheckRequest request) {
        String rawText = valueOrEmpty(request == null ? null : request.rawRequirementText());
        List<MissingField> missingFields = new ArrayList<>();
        List<String> questions = new ArrayList<>();

        addIfMissing(missingFields, questions,
                hasText(request == null ? null : request.gradeLevel()) || GRADE_PATTERN.matcher(rawText).find(),
                "gradeLevel",
                "年级",
                "生成教学内容需要明确学生学段",
                "请补充本节课面向哪个年级学生？");
        ...
        addIfMissing(missingFields, questions,
                hasText(request == null ? null : request.subject()) || containsAny(rawText, KNOWN_SUBJECTS),
                "subject",
                "学科",
                "不同学科的知识组织方式和表达风格不同",
                "请补充本节课属于哪个学科？");
        ...
        addIfMissing(missingFields, questions,
                hasText(request == null ? null : request.topic()) || looksLikeTopicInRawText(rawText),
                "topic",
                "课题",
                "生成课件前需要明确具体课题",
                "请补充本节课的具体课题。");
        ...
        addIfMissing(missingFields, questions,
                hasText(request == null ? null : request.lessonDuration()) || DURATION_PATTERN.matcher(rawText).find(),
                "lessonDuration",
                "课时",
                "课时长度会影响内容容量和活动安排",
                "本节课预计多少分钟？");
        ...
        addIfMissing(missingFields, questions,
                hasText(request == null ? null : request.teachingGoals()),
                "teachingGoals",
                "教学目标",
        ...
        addIfMissing(missingFields, questions,
                hasOutputTypes(request == null ? null : request.outputTypes()),
                "outputTypes",
                "输出类型",
    ...
    private static boolean hasOutputTypes(List<String> outputTypes) {
        return outputTypes != null && outputTypes.stream().anyMatch(MockAIWorkflowGateway::hasText);
    }
