    ...
    >
      <template #actions>
        <el-tooltip content="刷新洞察" placement="bottom">
          <el-button circle :icon="Refresh" :loading="loading" aria-label="刷新学情洞察" @click="loadInsights" />
        </el-tooltip>
      </template>
    </PageHeader>
      ...
      <section class="surface-panel insight-section" aria-labelledby="class-scope-heading">
        ...
        <ul v-else class="class-list">
          <li v-for="item in classRows" :key="item.key">
            ...
            <el-button text type="primary" @click="openQuestionScope(item.publicationIds[0])">查看问答</el-button>
          </li>
        </ul>
      </section>
      ...
      <section class="insights-grid">
        <section class="surface-panel insight-section" aria-labelledby="publication-heading">
          <header class="insight-section__header">
            ...
            <el-button text type="primary" @click="goTo(publicationRoute)">查看发布</el-button>
          </header>
          ...
          <ul v-else class="publication-list">
            <li v-for="item in publicationRows" :key="item.publication.id">
              ...
              <el-button text type="primary" @click="openQuestionScope(item.publication.id)">查看问答</el-button>
            </li>
          </ul>
        </section>
        ...
        <section class="surface-panel insight-section" aria-labelledby="topic-heading">
          <header class="insight-section__header">
            ...
            <el-button text type="primary" @click="goTo(questionRoute)">查看全部问答</el-button>
          </header>
          <StatePanel
          ...
          <ul v-else class="topic-list">
            <li v-for="item in topicRows" :key="item.title">
              ...
              <el-button text type="primary" @click="openQuestionScope(item.publicationId)">查看</el-button>
            </li>
          </ul>
        </section>
      </section>
...
const metrics = computed(() => [
  {
    ...
    action: '查看发布',
    destination: publicationRoute.value,
  },
  {
    key: 'publications',
    label: '有效发布',
  ...
  },
  {
    key: 'questions',
    label: '学生提问',
    value: sourceValue('学生问答', questionCount.value),
    description: '当前发布范围内实际收到的问题',
    action: '查看问答',
    destination: questionRoute.value,
  },
  {
    key: 'answered-rate',
    label: '已回答率',
    value: failedSources.value.includes('学生问答') ? '—' : answerRate.value,
    description: failedSources.value.includes('学生问答')
      ? '学生问答数据暂时不可用'
...
@media (max-width: 560px) {
  ...
  .class-list li > .el-button,
  .publication-list li > .el-button,
  .topic-list li > .el-button {
