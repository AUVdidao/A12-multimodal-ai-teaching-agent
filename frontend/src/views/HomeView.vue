<template>
  <section class="page dashboard-page">
    <header class="dashboard-heading">
      <div><h1>教师工作台</h1><p>从最近的教学项目继续备课工作。</p></div>
    ...
    <section class="dashboard-continue" aria-labelledby="continue-title">
      ...
      <template v-if="latestProject"><p>{{ projectMeta(latestProject) }}</p><div class="dashboard-continue__footer"><StatusBadge :status="latestProject.status" /><span>更新于 {{ formatDate(latestProject.updatedAt) }}</span><el-button type="primary" @click="openOverview(latestProject)">继续项目</el-button></div></template>
    ...
    <footer class="dashboard-status"><span class="dashboard-status__dot" /><span>{{ healthStatus }}</span><el-button text :icon="Refresh" :loading="checking" aria-label="重新检查服务状态" @click="checkHealth" /></footer>
  </section>
</template>
...
const healthStatus = ref('服务状态待检查');
...
onMounted(() => { loadProjects(); checkHealth(); });
...
async function checkHealth() { checking.value = true; try { healthStatus.value = (await checkBackendHealth()).data.status === 'UP' ? '服务正常' : '服务暂不可用'; } catch { healthStatus.value = '服务暂不可用'; } finally { checking.value = false; } }
function openOverview(project: TeachingProject) { router.push(`/projects/${project.id}/overview`); }
function nextTask(project: TeachingProject) { return project.status === 'REQUIREMENT_CONFIRMED' ? '查看需求摘要与下一步资料工作' : '继续完善教学需求'; }
...
.dashboard-heading { margin-bottom: 24px; }.dashboard-heading h1, .dashboard-heading p, h2, p { margin: 0; }.dashboard-heading h1 { font-size: 27px; }.dashboard-heading p { margin-top: 4px; color: var(--color-text-secondary); }
...
.dashboard-status { justify-content: flex-start; margin-top: 28px; color: var(--color-text-muted); font-size: 11px; }.dashboard-status__dot { width: 7px; height: 7px; border-radius: 50%; background: var(--color-success); }
@media (max-width: 860px) { .dashboard-grid { grid-template-columns: 1fr; } } @media (max-width: 560px) { .dashboard-heading, .dashboard-continue__footer { align-items: stretch; flex-direction: column; }.dashboard-continue__footer > span { margin-right: 0; }.recent-row__meta { align-items: flex-start; }.recent-row { align-items: flex-start; flex-direction: column; } }
</style>
